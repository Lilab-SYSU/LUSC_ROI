...
def Wfast(img,nstains,lamb,num_patches,patchsize,level,background_correction=False):
    ...
    _max=3000
    max_size=_max*_max
    if type(img) is openslide.ImageSlide:
        xdim,ydim=img.level_dimensions[level]
    else:
        xdim, ydim = img.size
    ...
    if ydim*xdim>max_size:
        ...
        for j in range(20):
            ...
            threshold=0.3 #maximum percentage of white pixels in patch
    ...
    if len(valid_inp)>0:
        WS = getstainMat(valid_inp[0],param,i0)
        WS=np.array(WS)
...
def suppress_stdout(out=None):
    if out is None:
        devnull = open('/public5/lilab/student/myang/project/lili/GG/test/color_norm.log', 'w')
